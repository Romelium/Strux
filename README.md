...
`markdown_processor` is a command-line utility written in Rust that parses specially formatted markdown files to generate directory structures and file contents. It's useful for:
...
2. Clone the repository:
    ...
    ```bash
    git clone https://github.com/???/markdown_processor.git
...
3. Build and install the binary:
    ...
    This will install the `markdown_processor` executable into your Cargo bin directory (usually `~/.cargo/bin/`).

### From Crates.io (Once Published)

```bash
cargo install markdown_processor
```

## Usage

```bash
markdown_processor [OPTIONS] <MARKDOWN_FILE>
```
...
* `// path/to/file.ext`: The header line is **included** in the file content. This is useful for self-documenting scripts.
    ...
    *(Resulting `scripts/run_analysis.py` will contain the `//scripts/...` line)*
...
Generated by Markdown Processor.
```
...
markdown_processor -o my_project example.md
...
```plaintext
Ensuring target base directory exists: my_project
Created base directory: my_project
Processing actions relative to resolved base: /path/to/your/cwd/my_project

Processing 5 actions in '/path/to/your/cwd/my_project'...

[1/5] Action: Create, Path: 'src/main.py'
  Creating directory: src
  Creating file: src/main.py (178 bytes)

[2/5] Action: Create, Path: 'src/utils.py'
  Creating file: src/utils.py (58 bytes)

[3/5] Action: Delete, Path: 'old_data.csv'
  Skipping deletion: File not found: old_data.csv

[4/5] Action: Create, Path: 'README.md'
  Creating file: README.md (46 bytes)

[5/5] Action: Delete, Path: 'temp/to_delete.log'
  Skipping deletion: File not found: temp/to_delete.log

[6/5] Action: Create, Path: '.gitignore'
  Creating file: .gitignore (20 bytes)
----------------------------------------
Processing Summary:
  Base Directory:                     /path/to/your/cwd/my_project
  Files created:                      4
  Files overwritten (--force):        0
  Files deleted:                      0
------------ Skipped ---------------
  Skipped (create, exists):           0
  Skipped (delete, not found):        2
  Skipped (delete, is dir):           0
  Skipped (delete, other type):       0
---------- Failed/Errors -----------
  Failed (unsafe/invalid path):       0
  Failed (create, target is dir):     0
  Failed (create, parent is file):    0
  Failed (I/O or Path error):         0
  Failed (other unexpected errors):   0
----------------------------------------
